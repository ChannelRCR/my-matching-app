...
import type { Deal, Invoice, User, Dispute } from '../types';
            ...
            <div id="pdf-page-2" style={pageContainerStyle}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #000', fontSize: '12px', marginBottom: '40px' }}>
                    <tbody>
                        ...
                        <tr><td style={{ border: '1px solid #000', padding: '8px', backgroundColor: '#f9f9f9' }}>振込先口座情報</td><td style={{ border: '1px solid #000', padding: '8px', whiteSpace: 'pre-wrap' }}>{buyer.bankAccountInfo || '未設定'}</td></tr>
                    </tbody>
                </table>
            </div>
...
export const generateSettlementPDFBlob = async (_deal: Deal, dispute: Dispute, seller: User, buyer: User): Promise<Blob> => {
    ...
    // settlement_amount or fallback to claim_amount
    const settlementAmount = dispute.settlement_amount || dispute.claim_amount || 0;
    const installmentsCount = dispute.installments_count || 1;
    const formattedAmount = settlementAmount.toLocaleString();
    ...
    const SettlementComponent = () => (
        <div style={{ fontFamily: '"Noto Serif JP", "Hiragino Mincho ProN", "MS PMincho", serif', color: '#000' }}>
            <div id="pdf-page-settlement-1" style={pageContainerStyle}>
                ...
                <div style={{ fontSize: '13px', marginBottom: '30px', lineHeight: 1.8, textIndent: '1em' }}>
                    甲（{sellerName}）と乙（{buyerName}）は、原契約（債権譲渡契約）に関する紛争について、本日以下の通り和解が成立したことを確認する。
                </div>
                ...
                <div style={{ fontSize: '13px', marginLeft: '20px', marginBottom: '20px', lineHeight: 1.8 }}>
                    {installmentsCount === 1 ? (
                        <>乙は甲に対し、解決金として金 {formattedAmount} 円を、別途合意する期日までに指定の口座に振り込んで支払う。</>
                    ) : (
                        <>乙は甲に対し、解決金として金 {formattedAmount} 円の支払義務があることを認め、これを {installmentsCount} 回に分割して、毎月末日までに各分割金を指定の口座に振り込んで支払う。</>
                    )}
                </div>
                ...
                {installmentsCount >= 2 && (
                    <>
                        ...
                        <div style={{ fontSize: '13px', marginLeft: '20px', marginBottom: '20px', lineHeight: 1.8 }}>
                            支払義務者が前条の分割金の支払いを1回でも怠ったときは、当然に期限の利益を失い、未払金残額およびこれに対する遅延損害金を直ちに一括して支払うものとする。
                        </div>
                    </>
                )}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '60px', padding: '0 20px' }}>
                    <div style={{ width: '45%' }}>
                        <div style={{ fontSize: '13px', marginBottom: '20px' }}>【甲】</div>
                    ...
                    <div style={{ width: '45%' }}>
                        <div style={{ fontSize: '13px', marginBottom: '20px' }}>【乙】</div>
